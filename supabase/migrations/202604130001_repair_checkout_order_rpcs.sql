...
create or replace function public.calculate_checkout_items_total_ugx(order_items jsonb)
returns integer
language sql
immutable
...
as $$
  select coalesce(
    sum(
      greatest(coalesce(item.price_ugx, 0), 0)
...
declare
  ...
  resolved_order_total_ugx integer := coalesce(
    order_total_ugx,
    public.calculate_checkout_items_total_ugx(order_items)
  );
begin
...
revoke all on function public.place_authenticated_order(
  uuid,
  uuid,
  text,
  integer,
...
grant execute on function public.place_authenticated_order(
  uuid,
  uuid,
  text,
  integer,
